...
def train(args_override):
    ...
    dataset = RealWorldDataset(
        ...
        with_cloud = False,
        with_obj_action = args.enable_mba,
    )
    ...
    if args.enable_mba:
        optimizer_mba = torch.optim.AdamW(policy.parameters(), lr = args.lr, betas = [0.95, 0.999], weight_decay = 1e-6)

        lr_scheduler_mba = get_cosine_schedule_with_warmup(
            optimizer = optimizer_mba,
            num_warmup_steps = 2000,
            num_training_steps = len(dataloader) * args.num_epochs
        )
        lr_scheduler_mba.last_epoch = len(dataloader) * (args.resume_epoch + 1) - 1
    ...
    for epoch in range(args.resume_epoch + 1, args.num_epochs):
        ...
        for data in pbar:
            ...
            if args.enable_mba:
                loss_mba = policy(cloud = cloud_data, 
                                   actions = action_data, 
                                   batch_size = action_data.shape[0],
                                   actions_obj = obj_data,
                                   sample_mba = True)
                ...
                optimizer_mba.step()
                optimizer_mba.zero_grad()
                lr_scheduler_mba.step()
                avg_loss_mba += loss_mba.item()

        avg_loss = avg_loss / num_steps
        sync_loss(avg_loss, device)
        train_history.append(avg_loss)
        if args.enable_mba:
            avg_loss_mba = avg_loss_mba / num_steps
            sync_loss(avg_loss_mba, device)
            train_history_mba.append(avg_loss_mba)

        if RANK == 0:
            print("Train loss: {:.6f}".format(avg_loss))
            if args.enable_mba:
                print("Train loss (MBA): {:.6f}".format(avg_loss_mba))
            ...
            if (epoch + 1) % args.save_epochs == 0:
                ...
                if args.enable_mba:
                    plot_history(train_history_mba, epoch, args.ckpt_dir, args.seed, identifier="load")
