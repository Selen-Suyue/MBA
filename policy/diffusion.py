...
class DiffusionUNetPolicy(nn.Module):
    def __init__(self,
        ...
        if enable_mba:
            ...
            self.fob=nn.Sequential(
                nn.Linear(obj_dim * horizon + obs_feature_dim, obs_feature_dim),
                nn.ReLU(),
    ...
    def predict_action(self, readout) -> Dict[str, torch.Tensor]:
        ...
        if self.enable_mba:
            ...
            global_cond = None
            global_cond = obs_features.reshape(B, -1)
            obj_pred = obj_pred.reshape(-1).reshape(global_cond.shape[0], -1)
    ...
    # ========= training  ============
    def compute_obj_loss(self, readout, actions_obj):
