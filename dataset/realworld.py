...
class RealWorldDataset(Dataset):
    ...
    def __init__(
        ...
        self.all_demos = sorted(os.listdir(self.data_path))[:NUM_DEMO] # use the first 50 demo
    ...
    def _augmentation(self, clouds, tcps, objs=None):
        ...
        if objs is not None:
            objs = apply_mat_to_pose(objs, aug_mat, rotation_rep = "quaternion")

        if objs is None:
            return clouds, tcps
        else:
            return clouds, tcps, objs
    ...
    def __getitem__(self, index):
        ...
        if timestamp not in self.projectors:
            ...
            self.projectors[timestamp] = Projector(os.path.join(CALIB_FILEDIR, timestamp))
        ...
        if self.split == 'train' and self.aug:
            if self.with_obj_action:
                clouds, action_tcps, action_objs = self._augmentation(clouds, action_tcps, action_objs)
            else:
